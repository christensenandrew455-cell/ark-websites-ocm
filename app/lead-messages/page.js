    ...
    return (
      <main className="min-h-screen bg-slate-100 p-0 text-slate-950 sm:p-4">
        <div className="mx-auto flex min-h-screen max-w-4xl flex-col overflow-hidden bg-white shadow-sm sm:min-h-[calc(100vh-2rem)] sm:rounded-3xl sm:border sm:border-slate-200">
          ...
          <form onSubmit={send} className="border-t border-slate-200 bg-white p-3 sm:p-4"><div className="flex items-end gap-2"><textarea required disabled={selected?.messagingOptedOut} rows={2} maxLength={1600} value={message} onChange={(event) => setMessage(event.target.value)} placeholder={selected?.messagingOptedOut ? "Customer opted out" : "Message"} className="min-h-12 flex-1 resize-none rounded-2xl border border-slate-300 px-4 py-3 text-sm outline-none focus:border-slate-950 disabled:bg-slate-100 disabled:text-slate-400" /><button disabled={sending || !selected?.leadPhone || selected?.messagingOptedOut} className="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-50">{sending ? "Sending…" : "Send"}</button></div></form>
        </div>
      </main>
    );
  }
