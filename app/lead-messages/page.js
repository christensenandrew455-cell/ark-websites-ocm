...
export default function LeadMessagesPage() {
  ...
  async function send(event) {
    ...
    if (selected?.messagingOptedOut) { setError("This customer opted out of text messages. They must text START before you can message them again."); return; }
  ...
  if (!featureEnabled) return <main className="min-h-[100dvh] bg-transparent p-4"><div className="mx-auto max-w-xl"><BackButton href="/" className="bg-slate-50" /><div className="mt-4 rounded-2xl border border-slate-300 bg-slate-100 p-5 text-sm font-bold leading-6 text-slate-700">{!MESSAGES_AVAILABLE ? `Messages are ${UPCOMING_FEATURE_LABEL.toLowerCase()}.` : "You do not currently have Messages turned on. Open Settings to enable it."}</div></div></main>;

  if (selectedLead) {
    const deleteKey = `${selectedCollection}:${selectedLead}`;
    return <main className="h-[calc(100dvh-7.75rem)] overflow-hidden bg-slate-200 p-0 text-slate-950 sm:h-[calc(100dvh-5.5rem)] sm:p-4"><div className="mx-auto flex h-full max-w-4xl flex-col overflow-hidden bg-slate-50 shadow-sm sm:rounded-3xl sm:border sm:border-slate-300"><div className="sticky top-0 z-20 shrink-0 border-b border-slate-300 bg-slate-100 px-3 py-3 sm:px-5"><div className="flex min-w-0 items-center gap-2"><BackButton onClick={closeConversation} className="shrink-0 bg-slate-50" label="Back" /><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-2"><h1 className="truncate text-base font-black sm:text-xl">{selected?.leadName || "Conversation"}</h1>{selected?.messagingOptedOut && <span className="shrink-0 rounded-full bg-red-100 px-2 py-1 text-[9px] font-black uppercase text-red-700">Opted out</span>}</div><p className="truncate text-[10px] font-bold text-slate-500 sm:text-xs">{selected?.leadPhone || "No phone number"}</p></div>{selected && !selected.newConversation && <button type="button" aria-label="Delete conversation" title="Delete conversation" disabled={Boolean(deleting)} onClick={() => deleteConversationRecord(selectedLead, selectedCollection, selected.leadName)} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-red-300 bg-slate-50 text-red-700 disabled:opacity-50">{deleting === deleteKey ? <span className="text-[10px] font-black">...</span> : <TrashIcon />}</button>}</div></div>{selected?.messagingOptedOut && <div className="shrink-0 border-b border-red-200 bg-red-50 px-4 py-3 text-xs font-bold leading-5 text-red-700">This customer replied STOP. Sending is disabled until the customer texts START to opt back in.</div>}{notice && <div className="shrink-0 border-b border-green-200 bg-green-50 px-4 py-2 text-xs font-bold text-green-800">{notice}</div>}{error && <div className="shrink-0 border-b border-red-200 bg-red-50 px-4 py-2 text-xs font-bold text-red-700">{error}</div>}{loading ? <div className="grid min-h-0 flex-1 place-items-center text-sm font-semibold text-slate-500">Loading conversation…</div> : <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain bg-slate-200/70 p-4 sm:p-6"><div className="flex min-h-full flex-col justify-end gap-3">{messages.map((item) => { const outbound = item.direction === "outbound"; const failed = outbound && (failedStatus(item.deliveryStatus) || item.providerError); const systemMessage = item.messageType === "opt-in-confirmation"; return <article key={item.id} className={outbound ? `ml-auto max-w-[85%] rounded-2xl rounded-br-md px-4 py-3 text-white shadow-sm ${systemMessage ? "bg-cyan-800" : "bg-slate-950"}` : "mr-auto max-w-[85%] rounded-2xl rounded-bl-md border border-slate-300 bg-slate-50 px-4 py-3 shadow-sm"}><p className="whitespace-pre-wrap text-sm leading-6">{item.body}</p><p className="mt-1 text-[10px] font-bold text-slate-400">{formatDate(item.createdAt)}{systemMessage ? " · consent notice" : ""}{item.deliveryStatus && outbound ? ` · ${statusLabel(item.deliveryStatus)}` : ""}</p>{failed && <p className="mt-1 text-[10px] font-bold leading-4 text-red-300">{item.providerError || "Message could not be delivered."}</p>}</article>; })}{messages.length === 0 && <div className="m-auto text-center"><p className="text-base font-black text-slate-700">No messages yet</p><p className="mt-1 text-xs text-slate-500">Send the first message to start this chat.</p></div>}</div></div>}<form onSubmit={send} className="sticky bottom-0 z-20 shrink-0 border-t border-slate-300 bg-slate-100 px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 sm:p-4"><div className="flex items-end gap-2"><textarea required disabled={selected?.messagingOptedOut} rows={2} maxLength={1600} value={message} onChange={(event) => setMessage(event.target.value)} placeholder={selected?.messagingOptedOut ? "Customer opted out" : "Message"} className="max-h-32 min-h-12 min-w-0 flex-1 resize-none rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-slate-950 disabled:bg-slate-200 disabled:text-slate-400" /><button disabled={sending || !selected?.leadPhone || selected?.messagingOptedOut} className="shrink-0 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white disabled:opacity-50">{sending ? "Sending…" : "Send"}</button></div></form></div></main>;
  }

  return <main className="min-h-[100dvh] bg-transparent px-3 py-4 text-slate-950 sm:p-6 md:p-8"><div className="mx-auto max-w-4xl"><div className="flex items-center justify-between gap-3"><BackButton href="/" className="bg-slate-50" /><button type="button" onClick={() => setShowContacts(true)} className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white">Contact Someone</button></div><header className="mt-5"><h1 className="text-4xl font-black tracking-tight">Messages</h1><p className="mt-2 text-sm font-semibold text-slate-600">Text clients from your dedicated business number.</p><div className="mt-4 inline-flex items-center gap-3 rounded-2xl border border-slate-300 bg-slate-100 px-4 py-3 text-sm font-black text-slate-700 shadow-sm"><span>{conversations.length.toLocaleString("en-US")} chats</span><span className="text-slate-400">·</span><span>{Number(data?.unreadCount || 0).toLocaleString("en-US")} unread</span></div></header>{notice && <div className="mt-4 rounded-xl border border-green-200 bg-green-50 p-3 text-sm font-bold text-green-800">{notice}</div>}{error && <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-700">{error}</div>}<div className="mt-5 space-y-3">{conversations.map((item) => { const unread = Number(item.unreadCount || 0); return <button key={`${item.collectionKey}:${item.leadId}`} type="button" onClick={() => openConversation(item.leadId, item.collectionKey)} className={unread > 0 ? "w-full rounded-2xl border border-slate-500 bg-white p-4 text-left shadow-md ring-1 ring-slate-300" : "w-full rounded-2xl border border-slate-300 bg-slate-100/70 p-4 text-left opacity-75 shadow-sm"}><div className="flex items-center justify-between gap-3"><h2 className={unread > 0 ? "font-black text-slate-950" : "font-bold text-slate-700"}>{item.leadName || "Conversation"}</h2>{unread > 0 && <span className="min-w-6 rounded-full bg-slate-950 px-2 py-1 text-center text-[10px] font-black text-white">{unread}</span>}</div><p className={unread > 0 ? "mt-1 truncate text-xs font-bold text-slate-700" : "mt-1 truncate text-xs font-semibold text-slate-500"}>{item.lastMessage || "No messages yet"}</p></button>; })}{conversations.length === 0 && !loading && <p className="rounded-2xl border border-slate-300 bg-slate-50 p-8 text-center text-sm font-semibold text-slate-500">No conversations yet.</p>}</div>{showContacts && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 p-3"><button type="button" className="fixed inset-0" onClick={() => setShowContacts(false)} aria-label="Close" /><div className="relative mx-auto my-8 max-w-xl rounded-3xl bg-white p-5"><h2 className="text-2xl font-black">Contact Someone</h2><div className="mt-4 space-y-2">{availableLeads.map((item) => <button key={`${item.collectionKey}:${item.id}`} type="button" onClick={() => openConversation(item.id, item.collectionKey)} className="w-full rounded-xl border border-slate-300 p-3 text-left"><p className="font-black">{item.Name || item.name || "Client"}</p><p className="text-xs text-slate-500">{item.Phone || item.phone || "No phone number"}</p></button>)}{availableLeads.length === 0 && <p className="rounded-xl bg-slate-100 p-4 text-sm text-slate-500">No additional contacts are available.</p>}</div></div></div>}</div></main>;
}
