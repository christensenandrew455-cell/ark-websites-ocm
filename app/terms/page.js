...
const sections = [
  ...
  },
  {
    ...
    body: <>
      ...
      <p><strong>First-payment refund window:</strong> For a subscription purchased directly on the ARK website through Stripe, the owner may request a refund of the first subscription charge within seven calendar days after that charge by using Help. This first-payment window does not apply to renewals, lead top-ups, custom work, provider pass-through charges, taxes, previously refunded accounts, or purchases made through Apple. ARK may deny requests involving fraud or abuse and will honor rights required by applicable law. If ARK approves a refund, it is returned through the original payment method and the associated subscription and paid service may end. Apple controls eligibility and processing for refunds of App Store purchases, which must be requested from Apple.</p>
      ...
      <p><strong>Payment display:</strong> Settings shows only the current plan, accepted leads used and remaining, top-ups, and payment method. Manage Plan &amp; Payment contains the available plans, explicit change confirmation, top-up purchase, and payment-method control. Apple’s or Stripe’s payment records control completed purchases, renewal timing, and charges.</p>
  ...
  {
    id: "rewards",
    title: "Refer & Save referral reward",
    body: <><p>Refer &amp; Save is a one-time new-account offer. An eligible Stripe-billed owner has 24 hours after account activation to refer one different business. That business must use the owner’s code during signup and complete its first paid subscription signup before the 24-hour window ends. The offer ends permanently after one qualifying referral or when the 24 hours expire.</p><p>One qualifying referral creates a USD credit equal to the recurring price of the referrer’s current plan when the referral qualifies, after any active recurring account discount. For example, if the referrer is paying $169.99 per month for Scale when the referral qualifies, the reward credit is $169.99. ARK sends the credit to the referrer’s Stripe customer balance so it begins applying automatically to the next eligible Stripe bill. If a later plan change makes the available credit greater than the next bill, the unused Stripe balance remains available for a later eligible Stripe bill.</p><p>Apple-billed accounts are not eligible until ARK separately releases an Apple-compliant in-app promotional-offer flow. A referral reward has no cash value, cannot be transferred, and cannot be withdrawn. Self-referrals, duplicate or fabricated accounts, late signups, signups without the code, invalid or unpaid accounts, fraud, abuse, or a reversed qualifying payment do not qualify. ARK may deny or remove an unused reward when those conditions are not met.</p><p>Feedback does not earn a billing reward.</p></>,
  },
  ...(MESSAGES_AVAILABLE ? [{
    id: "messages",
