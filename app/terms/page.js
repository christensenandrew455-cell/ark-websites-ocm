...
const sections = [
  ...
  },
  {
    ...
    body: <><p>An eligible owner may share the referral code or link shown in Refer &amp; Save. A referral qualifies after a different business uses that code during signup, creates a valid account, and completes its first paid subscription signup. Each referred business can qualify once. There is no three-referral monthly cap.</p><p>For an eligible Stripe-billed referring account, one qualifying referral creates a USD credit equal to the recurring price of the referrer’s current plan when the referral qualifies, after any active recurring account discount. For example, if the referrer is paying $169.99 per month for Scale when the referral qualifies, the reward credit is $169.99. ARK sends the credit to the referrer’s Stripe customer balance so it begins applying automatically to the next eligible Stripe bill. If multiple rewards or a later plan change make the available credit greater than the next bill, the unused Stripe balance remains available for later eligible Stripe bills.</p><p>Refer &amp; Save is initially available only to Stripe-billed accounts. Apple-billed accounts are not eligible until ARK separately releases an Apple-compliant in-app promotional-offer flow. A referral reward has no cash value, cannot be transferred, and cannot be withdrawn. Self-referrals, duplicate or fabricated accounts, signups without the code, invalid or unpaid accounts, fraud, abuse, or a reversed qualifying payment do not qualify. ARK may deny or remove an unused reward when those conditions are not met.</p><p>Feedback does not earn a billing reward.</p></>,
  },
  ...(MESSAGES_AVAILABLE ? [{
    id: "messages",
