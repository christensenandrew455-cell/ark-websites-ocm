import { Environment, SignedDataVerifier } from "@apple/app-store-server-library";
import { APPLE_IAP_BUNDLE_ID } from "./appleIapCatalog.js";

// Apple public root certificates, DER encoded. Keep these in sync with
// https://www.apple.com/certificateauthority/ when Apple publishes a new root.
const APPLE_ROOT_CERTIFICATES_BASE64 = [
  "",
  "",
  "",
];

const roots = APPLE_ROOT_CERTIFICATES_BASE64.map((certificate) => Buffer.from(certificate, "base64"));
const verifierCache = new Map();

function text(value) { return String(value || "").trim(); }

export function unsafeAppleJwsPayload(signedPayload) {
  const encoded = text(signedPayload).split(".")[1];
  if (!encoded) throw new Error("APPLE_IAP_JWS_INVALID");
  try {
    return JSON.parse(Buffer.from(encoded, "base64url").toString("utf8"));
  } catch {
    throw new Error("APPLE_IAP_JWS_INVALID");
  }
}

function environmentFor(value) {
  if (value === Environment.PRODUCTION) return Environment.PRODUCTION;
  if (value === Environment.SANDBOX) return Environment.SANDBOX;
  throw new Error("APPLE_IAP_ENVIRONMENT_INVALID");
}

export function appleVerifier(environmentValue) {
  const environment = environmentFor(environmentValue);
  if (verifierCache.has(environment)) return verifierCache.get(environment);
  const rawAppleId = text(process.env.APPLE_IAP_APPLE_ID);
  const appAppleId = rawAppleId ? Number(rawAppleId) : undefined;
  if (environment === Environment.PRODUCTION && (!Number.isSafeInteger(appAppleId) || appAppleId <= 0)) {
    throw new Error("APPLE_IAP_APPLE_ID_REQUIRED");
  }
  const verifier = new SignedDataVerifier(
    roots,
    false,
    environment,
    APPLE_IAP_BUNDLE_ID,
    environment === Environment.PRODUCTION ? appAppleId : undefined,
  );
  verifierCache.set(environment, verifier);
  return verifier;
}

export async function verifySignedAppleTransaction(signedTransaction) {
  const hint = unsafeAppleJwsPayload(signedTransaction);
  const environment = environmentFor(hint.environment);
  const transaction = await appleVerifier(environment).verifyAndDecodeTransaction(text(signedTransaction));
  return { environment, transaction };
}

export async function verifySignedAppleNotification(signedPayload) {
  const hint = unsafeAppleJwsPayload(signedPayload);
  const environment = environmentFor(hint.data?.environment);
  const verifier = appleVerifier(environment);
  const notification = await verifier.verifyAndDecodeNotification(text(signedPayload));
  return { environment, notification, verifier };
}
