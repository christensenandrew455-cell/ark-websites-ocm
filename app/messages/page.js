...
function CustomerMessages({ user, requests, onRefresh }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setMessage("");
      setNotice(type === "help"
        ? "Priority help request sent. It has been marked as priority."
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-3 py-4 text-slate-950 sm:p-6 md:p-8">
      <div className="mx-auto max-w-3xl">
        <header className="mb-4 sm:mb-7"><h1 className="text-3xl font-black tracking-tight sm:text-4xl">Requests</h1></header>
        ...
        <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-7">
          ...
          <div className={type === "help" ? "mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-semibold leading-5 text-red-800" : "mt-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs font-semibold leading-5 text-blue-800"}>
            {type === "help"
              ? "Priority help is only for a broken receptionist, missing lead data, or another issue that is seriously affecting the business."
              : "Use this for wording, voice, business information, hours, or other receptionist changes."}
          </div>
          <label className="mt-4 block"><span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Subject</span><input value={subject} onChange={(event) => setSubject(event.target.value)} placeholder={type === "help" ? "What is broken?" : "What should change?"} className="mt-1.5 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm outline-none focus:border-slate-950" /></label>
          <label className="mt-3 block"><span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Details</span><textarea rows={6} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Explain what is happening and what you need." className="mt-1.5 w-full rounded-xl border border-slate-300 p-3 text-sm outline-none focus:border-slate-950" /></label>
        ...
        <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:mt-6 sm:rounded-3xl sm:p-7">
          <h2 className="text-lg font-black">Your Request History</h2>
          <div className="mt-3 space-y-2">
            {requests.map((item) => (
              <article key={item.id} className="rounded-xl border border-slate-200 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0"><p className="truncate text-sm font-black">{item.subject}</p><p className="mt-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-400">{item.type === "help" ? "Priority Help" : "Change"} · {formatDate(item.createdAt)}</p></div>
      ...
      <div className="mx-auto max-w-5xl">
        <header className="mb-4 flex items-end justify-between gap-3 sm:mb-7">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={onRefresh} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-xs font-black">Refresh</button>
          </div>
        </header>
        {error && <div className="mb-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-700">{error}</div>}
        <div className="grid grid-cols-3 gap-1 rounded-xl bg-slate-100 p-1">
          {[["open", "Open"], ["help", "Help"], ["change", "Changes"]].map(([value, label]) => (
          ...
          {visible.map((item) => {
            ...
            return (
              <article key={item.id} className={item.type === "help" ? "rounded-2xl border border-red-200 bg-white p-4 shadow-sm" : "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="text-base font-black">{item.subject}</h2><span className={item.type === "help" ? "rounded-full bg-red-100 px-2 py-1 text-[9px] font-black uppercase text-red-700" : "rounded-full bg-blue-100 px-2 py-1 text-[9px] font-black uppercase text-blue-700"}>{item.type === "help" ? "Priority Help" : "Change"}</span></div><p className="mt-1 text-xs font-semibold text-slate-500">{item.businessName} · {item.ownerName || item.accountEmail} · {formatDate(item.createdAt)}</p></div>
                  <StatusBadge status={item.status} />
                </div>
                ...
                {isNew ? (
                  <div className="mt-3 grid grid-cols-2 gap-2"><button disabled={isSaving} onClick={() => update(item, "denied")} className="rounded-xl border border-red-300 px-3 py-2.5 text-xs font-black text-red-700 disabled:opacity-50">Deny</button><button disabled={isSaving} onClick={() => update(item, "in-progress")} className="rounded-xl bg-blue-700 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">{isSaving ? "Saving…" : "Start"}</button></div>
                ) : (
                  <button disabled={isSaving} onClick={() => update(item, "completed")} className="mt-3 w-full rounded-xl bg-green-700 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">{isSaving ? "Saving…" : "Complete"}</button>
                )}
              </article>
            );
          })}
          {visible.length === 0 && <p className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500 shadow-sm">No unresolved requests in this view.</p>}
        </section>
      </div>
    </main>
  );
