...
function StatusPill({ account }) {
  if (account.billing?.phase === "deletion-review") return <span className="rounded-full bg-red-100 px-2.5 py-1 text-[9px] font-black uppercase text-red-700">Deletion Review</span>;
  if (account.billing?.restricted) return <span className="rounded-full bg-orange-100 px-2.5 py-1 text-[9px] font-black uppercase text-orange-800">Payment Restricted</span>;
  if (account.billing?.phase === "grace") return <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[9px] font-black uppercase text-amber-800">Payment Due</span>;
  if (account.status === "disabled") return <span className="rounded-full bg-slate-200 px-2.5 py-1 text-[9px] font-black uppercase text-slate-700">Disabled</span>;
...
function RequestStatus({ status }) {
  const classes = status === "completed" ? "bg-green-100 text-green-800" : status === "denied" ? "bg-red-100 text-red-700" : status === "in-progress" ? "bg-blue-100 text-blue-800" : "bg-amber-100 text-amber-800";
  return <span className={`rounded-full px-2 py-1 text-[9px] font-black uppercase ${classes}`}>{status}</span>;
}
...
function LegalAgreementPanel({ account }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-8">
      <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Account evidence</p><h2 className="mt-1 text-lg font-black sm:text-2xl">Legal Agreement</h2></div><span className={accepted ? "rounded-full bg-green-100 px-2.5 py-1 text-[9px] font-black uppercase text-green-800" : "rounded-full bg-amber-100 px-2.5 py-1 text-[9px] font-black uppercase text-amber-800"}>{accepted ? "Accepted" : "Not Recorded"}</span></div>
      {accepted ? (
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Terms</p><Link href="/terms" target="_blank" className="mt-1 inline-block text-sm font-black text-blue-700 underline">Version {account.termsVersion || "not labeled"}</Link></div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Privacy</p><Link href="/privacy" target="_blank" className="mt-1 inline-block text-sm font-black text-blue-700 underline">Version {account.privacyVersion || "not labeled"}</Link></div>
      ...
      ) : <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-900">No signup agreement record is stored for this account.</p>}
    </section>
  );
}
...
export default function ConnectionsPage() {
  ...
  async function saveConnection(event, regenerateKey = false) {
    ...
    try {
      ...
      setMessage(regenerateKey ? "The connection key was regenerated." : "Customer setup saved.");
    } catch (saveError) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {!showCreate && !selectedId && (
          <section className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {businesses.map((business) => <button key={business.clientId} type="button" onClick={() => { setSelectedId(business.clientId); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm"><div className="min-w-0"><span className="block truncate text-sm font-black">{business.businessName}</span><span className="mt-0.5 block truncate text-xs font-semibold text-slate-500">{business.ownerName || business.accountEmail}</span></div><StatusPill account={business} /></button>)}
          </section>
        )}
        ...
        {selectedId && (
          <form onSubmit={saveConnection} className="space-y-3 sm:space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-xl font-black sm:text-3xl">{form.businessName}</h2><StatusPill account={form} /></div><p className="mt-0.5 font-mono text-[10px] text-slate-500">{selectedId}</p></div><button type="button" onClick={() => setSelectedId("")} className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-bold">Close</button></div>
            ...
            {form.billing?.showNotice && <section className={form.billing.phase === "deletion-review" ? "rounded-2xl border border-red-200 bg-red-50 p-4 shadow-sm sm:p-6" : "rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm sm:p-6"}><h2 className="text-lg font-black">Payment Status</h2><div className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><p className="text-[10px] font-black uppercase text-slate-500">Phase</p><p className="font-black">{form.billing.phase.replaceAll("-", " ")}</p></div><div><p className="text-[10px] font-black uppercase text-slate-500">Amount due</p><p className="font-black">{formatMoney(form.billing.amountDue, form.billing.currency)}</p></div><div><p className="text-[10px] font-black uppercase text-slate-500">Incident</p><p className="font-black">{form.billing.offenseNumber} in six months</p></div><div><p className="text-[10px] font-black uppercase text-slate-500">Review date</p><p className="font-black">{formatDate(form.billing.reviewAt)}</p></div></div></section>}

            <LegalAgreementPanel account={form} />

            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8"><h2 className="text-lg font-black">Private Connection Key</h2><div className="mt-4 flex gap-2"><input readOnly value={form.connectionKey || "No connection key saved"} className="h-10 min-w-0 flex-1 rounded-xl border border-slate-300 bg-slate-50 px-3 font-mono text-[10px]" /><button type="button" onClick={() => copy(form.connectionKey)} className="rounded-xl border border-slate-300 px-4 text-xs font-bold">Copy</button></div><button type="button" disabled={isSaving || form.status === "disabled"} onClick={() => saveConnection(null, true)} className="mt-3 rounded-xl border border-red-300 px-3 py-2 text-xs font-black text-red-700 disabled:opacity-50">Regenerate Key</button></section>

            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8"><div className="flex items-center justify-between"><h2 className="text-lg font-black">Request History</h2><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black">{requestHistory.length}</span></div><div className="mt-4 space-y-2">{requestHistory.map((item) => <article key={item.id} className="rounded-xl border border-slate-200 p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black">{item.subject}</p><p className="mt-0.5 text-[10px] font-bold uppercase text-slate-400">{item.type} · {formatDate(item.createdAt)}</p></div><RequestStatus status={item.status} /></div><p className="mt-2 text-xs leading-5 text-slate-600">{item.message}</p>{item.adminNote && <p className="mt-2 rounded-lg bg-slate-50 p-2 text-xs font-semibold">ARK: {item.adminNote}</p>}</article>)}{requestHistory.length === 0 && <p className="rounded-xl bg-slate-50 p-5 text-center text-sm text-slate-500">No requests for this account.</p>}</div></section>

            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8"><h2 className="text-lg font-black">Account Control</h2><p className="mt-1 text-xs leading-5 text-slate-500">Payment restriction is automated. Permanent deletion is always your manual decision.</p><div className="mt-4 grid grid-cols-3 gap-2">{form.status === "disabled" ? <button type="button" disabled={lifecycleBusy} onClick={() => lifecycleAction("restore")} className="rounded-xl bg-green-700 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">Restore</button> : <button type="button" disabled={lifecycleBusy} onClick={() => lifecycleAction("disable")} className="rounded-xl border border-amber-300 px-3 py-2.5 text-xs font-black text-amber-800 disabled:opacity-50">Disable</button>}<button type="button" disabled={lifecycleBusy} onClick={() => lifecycleAction("delete-now")} className="rounded-xl bg-red-600 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">Delete Permanently</button><button type="submit" disabled={isSaving || form.status === "disabled"} className="rounded-xl bg-slate-950 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">{isSaving ? "Saving…" : "Save Profile"}</button></div></section>
          </form>
        )}
      </div>
    </main>
