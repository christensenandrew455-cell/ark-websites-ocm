    ...
    <div>
      ...
      <div className="mt-1.5 flex items-center gap-2 sm:mt-2">
        ...
        <button
          ...
          className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-bold disabled:opacity-40 sm:px-4 sm:py-2.5 sm:text-sm"
        >
          Copy
        </button>
      </div>
...
export default function ConnectionsPage() {
  ...
  if (loading || isLoading) {
    return <main className="grid min-h-[70vh] place-items-center p-6 text-sm font-semibold text-slate-500">Loading customers…</main>;
  }
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-4 flex items-end justify-between gap-3 sm:mb-8">
          <div>
            ...
            <h1 className="mt-1.5 text-3xl font-black tracking-tight sm:mt-2 sm:text-4xl">Customers</h1>
          </div>
          <button
            type="button"
            onClick={() => {
            ...
            className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white sm:px-5 sm:py-3 sm:text-sm"
          >
        ...
        {showCreate && (
          <form onSubmit={createCustomer} className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:mb-7 sm:rounded-3xl sm:p-8">
            <div className="mb-4 sm:mb-6">
              <h2 className="text-xl font-black sm:text-2xl">New Customer</h2>
              <p className="mt-1 text-xs text-slate-500 sm:mt-2 sm:text-sm">Creates the login, isolated OCM account, and private connection key.</p>
            </div>
            ...
            <div className="mt-4 flex justify-end sm:mt-6">
              <button disabled={isCreating} className="rounded-xl bg-slate-950 px-5 py-2.5 text-xs font-black text-white disabled:opacity-50 sm:px-6 sm:py-3 sm:text-sm">{isCreating ? "Creating…" : "Create Customer"}</button>
            </div>
          </form>
        )}
        ...
        {!showCreate && !selectedId && (
          <>
            <button type="button" className="mb-3 w-full rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm sm:mb-5 sm:p-6">
              <p className="text-4xl font-black text-slate-950">{businesses.length}</p>
              <h2 className="mt-1 text-sm font-black">Customer Accounts</h2>
              <p className="mt-1 text-[10px] font-bold text-slate-400 sm:text-xs">Tap a customer below to open their setup</p>
            </button>
            ...
            <section className="grid gap-2 sm:grid-cols-2 sm:gap-3 lg:grid-cols-3">
              {businesses.map((business) => (
                <button
                  ...
                  type="button"
                  onClick={() => openCustomer(business.clientId)}
                  className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left shadow-sm active:scale-[0.99] sm:rounded-2xl sm:p-4"
                >
                  <div className="min-w-0">
                    <span className="block truncate text-sm font-black">{business.businessName}</span>
                    <span className="mt-0.5 block truncate text-xs font-semibold text-slate-500">{business.ownerName || business.accountEmail || business.clientId}</span>
                  </div>
                  <span className={business.enabled === false ? "h-2.5 w-2.5 shrink-0 rounded-full bg-red-500" : "h-2.5 w-2.5 shrink-0 rounded-full bg-green-500"} aria-label={business.enabled === false ? "Disabled" : "Enabled"} />
                </button>
              ))}
          ...
          <form onSubmit={saveConnection} className="space-y-3 sm:space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-8">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Customer</p>
                  <h2 className="mt-1 truncate text-xl font-black sm:text-3xl">{form.businessName}</h2>
                  <p className="mt-0.5 truncate font-mono text-[10px] text-slate-500 sm:text-xs">{selectedId}</p>
                </div>
              ...
              <label className="mt-4 flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-black sm:px-4 sm:py-3 sm:text-sm">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-8">
              ...
              <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900 sm:mt-6 sm:rounded-2xl sm:p-4 sm:text-sm sm:leading-6">
                Regenerating stops the old key immediately. Replace it in Railway before the next call.
              </div>
              <button type="button" disabled={isSaving} onClick={regenerateConnectionKey} className="mt-3 rounded-xl border border-red-300 px-3 py-2 text-xs font-black text-red-700 disabled:opacity-50 sm:mt-4 sm:px-4 sm:py-2.5 sm:text-sm">Regenerate Key</button>
            </section>

            <div className="flex justify-end">
              <button type="submit" disabled={isSaving} className="w-full rounded-xl bg-slate-950 px-6 py-3 text-sm font-black text-white disabled:opacity-50 sm:w-auto">{isSaving ? "Saving…" : "Save Customer"}</button>
            </div>
          </form>
        )}
      </div>
