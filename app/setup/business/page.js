...
export default function BusinessSetupPage() {
  ...
  async function cancelSignup() {
    ...
    try {
      ...
      await readApiJson(response, "Unable to cancel this temporary signup.");
      await signOut(auth);
      window.location.replace("/signup");
    } catch (cancelError) {
      setError(publicFormError(cancelError, "Unable to cancel this temporary signup."));
      setSaving(false);
    }
  }
      ...
      <section className="mx-auto w-full max-w-4xl rounded-3xl bg-white p-6 shadow-2xl sm:p-9">
        ...
        <form onSubmit={continueSignup} className="mt-7">
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={cancelSignup} disabled={saving} className="rounded-xl border border-slate-300 px-5 py-3 text-center text-sm font-black text-slate-700 disabled:opacity-60">Back</button>
