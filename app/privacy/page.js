...
const sections = [
  { title: "What this policy covers", body: <p>This Privacy Policy explains how ARK Websites collects, uses, stores, discloses, exports, and deletes information through ARK Client Center and its related AI receptionist, lead-management{MESSAGES_AVAILABLE ? ", optional messaging" : ""}, support, website, and billing services.</p> },
  {
    ...
    body: <>
      ...
      <p><strong>Billing and usage information:</strong> Apple account-token, product, subscription, original-transaction, transaction, expiration, environment, and notification identifiers; Stripe Customer, SetupIntent, PaymentMethod, subscription, PaymentIntent, coupon, invoice, and payment-event identifiers; the current usage-point or credit balance{MESSAGES_AVAILABLE ? "; rolling SMS-part remainder" : ""}; purchase or charge state; last successful payment time; payment-retry and deletion deadlines; and referral qualification. Apple or Stripe controls sensitive payment details, and ARK does not receive or store full card numbers, expiration values, security codes, or Apple Account credentials.</p>
  ...
  },
  {
    ...
    body: <>
      ...
      <p>Apple or Stripe payment events may be used to activate and renew the monthly account, verify user-confirmed Apple usage-credit purchases or collect authorized Stripe usage charges, prevent duplicate credit, show payment notices, pause paid services after a subscription failure, restore service, or delete a payment-restricted account after the seven-day recovery window.</p>
    </>,
  },
  {
    title: "Payment setup",
    body: <><p>During signup, ARK verifies the email address and phone number before collecting business information. On iOS, StoreKit sends purchase details to Apple; ARK verifies Apple’s signed transaction, starts the $50 monthly subscription, and promotes the temporary signup into a regular account. On other supported platforms, Stripe’s Payment Element sends sensitive payment information directly to Stripe; ARK verifies the SetupIntent, starts the $50 monthly subscription, and promotes the temporary signup into a regular account that is already verified.</p><p>Stripe-billed accounts authorize later off-session usage charges. Apple-billed accounts are shown Apple’s confirmation sheet for each consumable usage-credit purchase; ARK cannot initiate that Apple purchase without user confirmation. Apple and Stripe may collect information needed to process payment under their own privacy practices.</p></>,
  },
  ...(MESSAGES_AVAILABLE ? [{
