...
const sections = [
  { title: "1. What this policy covers", body: <p>This Privacy Policy explains how ARK Websites collects, uses, stores, discloses, exports, and deletes information through ARK Client Center and its related AI receptionist, lead-management, optional messaging, support, website, and billing services.</p> },
  {
    ...
    body: <>
      ...
      <p><strong>Billing and usage information:</strong> Stripe Customer, SetupIntent, PaymentMethod, subscription, PaymentIntent, coupon, invoice, and payment-event identifiers; the current usage-point balance; rolling SMS-part remainder; charge state; last successful payment time; payment-retry and deletion deadlines; and referral qualification. Stripe controls the in-app payment fields, and ARK does not receive or store full card numbers, expiration values, or security codes.</p>
...
export default function PrivacyPage() {
  return <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-950 sm:px-6 sm:py-12"><div className="mx-auto max-w-4xl"><div className="mb-4"><LegalBackButton /></div><article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10"><LegalPageHeader title="Privacy Policy" effectiveDate={PRIVACY_EFFECTIVE_DATE} version={PRIVACY_VERSION} active="privacy" /><div className="mt-7 space-y-8 text-sm leading-7 text-slate-700 sm:text-base">{sections.map((section) => <section key={section.title}><h2 className="text-xl font-black tracking-tight text-slate-950 sm:text-2xl">{section.title}</h2><div className="mt-2 space-y-3">{section.body}</div></section>)}</div></article></div></main>;
}
