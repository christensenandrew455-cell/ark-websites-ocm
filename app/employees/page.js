...
const DIRECTORY_LABELS = { name: "Employee name", email: "Employee email", phone: "Employee phone number" };
...
export default function EmployeesPage() {
  ...
  async function saveAccess() {
    ...
    setNotice(""); setError("");
    try {
      const result = await employeeApi(user, { method: "POST", body: JSON.stringify({ action: "access", visibility, directoryVisibility, employeeMessagingEnabled }) });
      const nextAccess = {
      ...
      setEmployeeMessagingEnabled(nextAccess.employeeMessagingEnabled);
      setSavedAccess(nextAccess);
      setWorkspace((current) => current ? { ...current, employeeVisibility: nextAccess.visibility, employeeDirectoryVisibility: nextAccess.directoryVisibility, employeeMessagingEnabled: nextAccess.employeeMessagingEnabled } : current);
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-3 py-4 text-slate-950 sm:p-6 md:p-8"><div className="mx-auto max-w-6xl">
      ...
      <header className="mt-4 sm:mt-6"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Account workspace</p><h1 className="mt-1 text-3xl font-black tracking-tight sm:text-4xl">Employees</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Approve or delete employee accounts, control access, and assign work. Each active employee is {money(workspace?.perEmployeeCents || 500)} per billing period.</p></header>
      ...
      <section className="mt-5 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Pending requests</p><h2 className="mt-1 text-xl font-black sm:text-2xl">Accept Employees</h2><p className="mt-1 text-xs font-semibold text-slate-500">Pending accounts are free until approved and active.</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{pendingEmployees.map((employee) => <article key={employee.uid} className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><h3 className="font-black">{employee.name}</h3><p className="mt-1 text-xs font-semibold text-slate-600">{employee.email}</p><p className="mt-1 text-xs font-semibold text-slate-600">{employee.phone}</p><div className="mt-4 grid grid-cols-2 gap-2"><button disabled={Boolean(busy)} onClick={() => runAction({ action: "approve", employeeUid: employee.uid }, `${employee.name} can now sign in.`, "Employees")} className="rounded-xl bg-green-700 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">Approve</button><button disabled={Boolean(busy)} onClick={() => deleteEmployee(employee)} className="rounded-xl border border-red-300 bg-white px-3 py-2.5 text-xs font-black text-red-700 disabled:opacity-50">Delete</button></div></article>)}{pendingEmployees.length === 0 && <p className="rounded-2xl bg-slate-50 p-6 text-center text-sm font-semibold text-slate-500 sm:col-span-2">No employee accounts are waiting for approval.</p>}</div></section>

      <section className="mt-5 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Active and disabled</p><h2 className="mt-1 text-xl font-black sm:text-2xl">Employees</h2><p className="mt-1 text-xs font-semibold text-slate-500">{workspace?.activeEmployeeCount || 0} active · {money((workspace?.activeEmployeeCount || 0) * (workspace?.perEmployeeCents || 500))} this period</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{managedEmployees.map((employee) => <article key={employee.uid} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-black">{employee.name}</h3><p className="mt-1 truncate text-xs font-semibold text-slate-500">{employee.email}</p><p className="mt-1 text-xs font-semibold text-slate-500">{employee.phone}</p></div><span className={employee.status === "active" ? "rounded-full bg-green-100 px-2.5 py-1 text-[10px] font-black uppercase text-green-800" : "rounded-full bg-red-100 px-2.5 py-1 text-[10px] font-black uppercase text-red-700"}>{employee.status}</span></div><div className="mt-4 grid grid-cols-2 gap-2">{employee.status === "active" ? <button disabled={Boolean(busy)} onClick={() => runAction({ action: "disable", employeeUid: employee.uid }, `${employee.name} was disabled.`, "Employees")} className="rounded-xl border border-slate-300 px-3 py-2.5 text-xs font-black text-slate-700 disabled:opacity-50">Disable</button> : <button disabled={Boolean(busy)} onClick={() => runAction({ action: "activate", employeeUid: employee.uid }, `${employee.name} was reactivated.`, "Employees")} className="rounded-xl bg-slate-950 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">Reactivate</button>}<button disabled={Boolean(busy)} onClick={() => deleteEmployee(employee)} className="rounded-xl border border-red-300 px-3 py-2.5 text-xs font-black text-red-700 disabled:opacity-50">Delete</button></div></article>)}{managedEmployees.length === 0 && <p className="rounded-2xl bg-slate-50 p-6 text-center text-sm font-semibold text-slate-500 sm:col-span-2">No approved employees yet.</p>}</div></section>

      <section id="visibility" className="mt-5 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Employee access</p><h2 className="mt-1 text-xl font-black sm:text-2xl">Employee Access Settings</h2><p className="mt-2 text-xs leading-5 text-slate-500">Choose whether employees can use Messages, what assigned lead information they can see, and what coworker contact information appears in employee Settings.</p>
        <label className={workspace?.messagesEnabled ? "mt-4 flex items-center justify-between gap-4 rounded-xl border border-slate-200 p-4" : "mt-4 flex items-center justify-between gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4 opacity-60"}><span><strong className="block text-sm">Messages for Employees</strong><span className="text-xs text-slate-500">Allow approved employees to message only the leads assigned to them.{workspace?.messagesEnabled ? "" : " Turn on Messages in owner Settings first."}</span></span><input type="checkbox" disabled={!workspace?.messagesEnabled} checked={employeeMessagingEnabled} onChange={(event) => setEmployeeMessagingEnabled(event.target.checked)} className="h-5 w-5 accent-slate-950" /></label>
        <h3 className="mt-6 text-sm font-black">Assigned Lead Information</h3><div className="mt-3 grid gap-2 sm:grid-cols-2">{Object.entries(VISIBILITY_LABELS).map(([key, label]) => <label key={key} className="flex items-center justify-between gap-4 rounded-xl border border-slate-200 p-3"><span className="text-sm font-bold text-slate-700">{label}</span><input type="checkbox" checked={visibility[key] === true} onChange={(event) => setVisibility((current) => ({ ...current, [key]: event.target.checked }))} className="h-5 w-5 accent-slate-950" /></label>)}</div>
        <h3 className="mt-6 text-sm font-black">Employee Directory Information</h3><p className="mt-1 text-xs leading-5 text-slate-500">These three controls decide what employees can see about coworkers. There is no leaderboard or performance comparison.</p><div className="mt-3 grid gap-2 sm:grid-cols-3">{Object.entries(DIRECTORY_LABELS).map(([key, label]) => <label key={key} className="flex items-center justify-between gap-4 rounded-xl border border-slate-200 p-3"><span className="text-sm font-bold text-slate-700">{label}</span><input type="checkbox" checked={directoryVisibility[key] === true} onChange={(event) => setDirectoryVisibility((current) => ({ ...current, [key]: event.target.checked }))} className="h-5 w-5 accent-slate-950" /></label>)}</div>
