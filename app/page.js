...
function SummaryCard({ label, value, href }) {
  return <Link href={href} className="block rounded-2xl border border-slate-200 bg-white p-4 shadow-sm active:scale-[0.99] sm:rounded-3xl sm:p-6"><p className="text-3xl font-black tracking-tight sm:text-4xl">{value}</p><h2 className="mt-1 text-xs font-black uppercase tracking-wide text-slate-700 sm:text-sm">{label}</h2></Link>;
}
...
function AdminDashboard({ user }) {
  ...
  const load = useCallback(async () => {
    try {
      const [dashboard, connections] = await Promise.all([adminApi(user, "/api/admin/dashboard"), adminApi(user, "/api/admin/connections")]);
      setData({ ...dashboard, counts: { ...(dashboard.counts || {}), customers: (connections.businesses || []).length } });
      setError("");
    } catch (loadError) {
  ...
  if (isLoading) return <main className="grid min-h-[70vh] place-items-center text-sm font-semibold text-slate-500">Loading dashboard…</main>;
  ...
  return (
    <main className="min-h-screen bg-transparent px-3 py-4 text-slate-950 sm:p-6 md:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-3 grid grid-cols-2 gap-3 sm:mt-4 sm:grid-cols-5 sm:gap-4"><SummaryCard label="Accounts" value={counts.customers || 0} href="/connections" /><SummaryCard label="Needs Numbers" value={counts.needsNumbers || 0} href="/connections" /><SummaryCard label="App Help" value={counts.openRequests || 0} href="/messages" /><SummaryCard label="Website" value={counts.websiteRequests || 0} href="/website-requests" /><SummaryCard label="Needs Payment" value={counts.needsPayment || 0} href="/payment" /></section>
      </div>
    </main>
  );
}
