...
export default function EmployeeAccessSettings({ onBack }) {
  ...
  if (profile?.employeesEnabled !== true) {
    return <main className="min-h-screen bg-slate-200 px-3 py-4 text-slate-950 sm:p-6 md:p-8"><div className="mx-auto max-w-4xl"><button type="button" onClick={onBack} aria-label="Back" className="grid h-12 w-12 place-items-center rounded-xl border border-slate-300 bg-slate-50 text-2xl font-black">←</button><div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm font-bold text-amber-800">Turn on Employees in Customization first.</div></div></main>;
  }
  ...
  return (
    <main className="min-h-screen bg-slate-200 px-3 py-4 text-slate-950 sm:p-6 md:p-8">
      ...
      <div className="mx-auto max-w-4xl">
        <button type="button" onClick={goBack} aria-label="Back" title="Back" className="grid h-12 w-12 place-items-center rounded-xl border border-slate-300 bg-slate-50 text-2xl font-black shadow-sm">←</button>
        ...
        <section className="mt-5 rounded-[2rem] border border-slate-300 bg-slate-300/70 p-3 shadow-inner sm:p-5">
          <div className="rounded-3xl border border-slate-300 bg-slate-50 p-4 shadow-sm sm:p-6">
            ...
            <h2 className="mt-6 text-lg font-black">Lead Information</h2>
            ...
            <h2 className="mt-6 text-lg font-black">Employee Directory</h2>
            ...
            <button type="button" disabled={saving || !dirty} onClick={save} className="mt-5 w-full rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-50 sm:w-auto">{saving ? "Saving…" : "Save"}</button>
          </div>
        </section>
      </div>
    </main>
