...
function DashboardCard({ value, label, description, onClick, disabled = false, tourId = "" }) {
  ...
  const content = (
    <div className="flex h-full items-start justify-between gap-4">
      <div className="min-w-0">
        ...
        <p className={disabled ? "mt-2 text-sm font-bold leading-5 text-blue-200" : "mt-2 text-sm font-semibold leading-5 text-blue-100"}>{description}</p>
      </div>
  ...
  if (disabled) {
    return <div aria-disabled="true" data-tour-id={tourId || undefined} className="min-h-28 w-full rounded-2xl border border-blue-300/20 bg-blue-950/45 p-5 text-left text-blue-50 shadow-sm sm:min-h-32 sm:rounded-3xl sm:p-6">{content}</div>;
  }

  return <button type="button" onClick={onClick} data-tour-id={tourId || undefined} className="min-h-28 w-full rounded-2xl border border-blue-300/30 bg-gradient-to-br from-blue-700 to-indigo-800 p-5 text-left text-white shadow-lg shadow-blue-950/15 transition active:scale-[0.99] sm:min-h-32 sm:rounded-3xl sm:p-6">{content}</button>;
}
  ...
  return (
    <section className="ark-dashboard-page min-h-[calc(100vh-78px)] bg-transparent px-3 py-4 text-slate-950 sm:px-5 sm:py-8 md:px-8">
      <div className="mx-auto max-w-6xl">
        <h1 className="px-1 text-3xl font-black tracking-tight text-white sm:text-4xl">Dashboard</h1>
        ...
        <section className={numberPending ? "mt-5 rounded-2xl border border-blue-200/80 bg-white p-4 shadow-lg shadow-blue-950/10 sm:rounded-3xl sm:p-5" : "mt-5 rounded-2xl border border-blue-100 bg-white p-4 shadow-lg shadow-blue-950/10 sm:rounded-3xl sm:p-5"}>
        ...
        <section className="mt-3 rounded-[2rem] border border-blue-200/30 bg-blue-950/55 p-3 shadow-inner sm:mt-5 sm:p-5">
