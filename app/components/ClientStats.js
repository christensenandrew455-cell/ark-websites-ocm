...
function insideRange(item, range) {
  if (range === "all") return true;
  const value = toMillis(item.createdAt || item.contactedAt || item.acceptedAt || item.updatedAt);
...
export default function ClientStats() {
  ...
  const leadCount = useMemo(() => {
    ...
    [...contacted, ...clients].forEach((item) => unique.set(item.id, item));
    return [...unique.values()].filter((item) => insideRange(item, range)).length;
  }, [clients, contacted, range]);
    ...
    <section className="min-h-[calc(100vh-78px)] bg-slate-50 px-3 py-5 text-slate-950 sm:px-5 sm:py-8 md:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 space-y-3 sm:space-y-4">
          <StatCard primary value={leadCount} label="Leads" subtitle="Open new leads and accepted clients" onClick={() => router.push("/leads")} />
