...
function contactedLeadOrder(left, right) {
  const urgencyDifference = Number(isEmergencyRequest(right)) - Number(isEmergencyRequest(left));
  return urgencyDifference || rowTime(left) - rowTime(right);
}
...
export default function ReviewClientsNative() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setContacted((data.contacted || []).map((item) => normalizeRow(item.id, item, "contactedMe")).sort(contactedLeadOrder));
