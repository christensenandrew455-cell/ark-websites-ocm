...
export default function AccountVerificationGate() {
  ...
  return <main className="ark-auth-page fixed inset-0 z-[200] grid min-h-screen place-items-center overflow-y-auto px-4 py-8">
    <section className="ark-auth-card w-full max-w-md rounded-[2rem] p-6 shadow-2xl sm:p-8" role="dialog" aria-modal="true" aria-labelledby="verification-title">
      ...
      <div className="mt-2 flex items-center gap-2"><h1 id="verification-title" className="text-3xl font-black tracking-tight">{expired ? "Verification time expired" : checking ? "Checking your account" : verified ? "Account verified" : editingContact ? "Correct your contact details" : status?.phoneRequired ? "Verify your email and phone" : "Verify your email"}</h1>{!expired && !checking && !verified && <InfoTip label="About verification">{editingContact ? "Saving makes the old codes stop working and sends new codes to both entries." : status?.phoneRequired ? "Enter the separate four-digit codes sent to your email and phone." : "Enter the four-digit code sent to your email."}</InfoTip>}</div>
