...
function CustomerWorkspace({ children, pathname, isPolicyPublic, profile, logout }) {
  ...
  return <><WorkspaceHeader profile={profile} pathname={pathname} isEmployee={false} logout={logout} /><PullToRefresh>{!isPolicyPublic && <LegalAcceptanceGate />}<PaymentNotice /><NativeAppSetup />{!status.restricted && <HelpCenter />}{children}</PullToRefresh>{!status.restricted && !pathname.startsWith("/lead-messages") && <ReferralCenter clientId={profile?.clientId} />}</>;
}
