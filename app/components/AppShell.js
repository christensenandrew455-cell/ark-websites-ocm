...
function formatDeadline(value) {
  ...
  return Number.isNaN(date.getTime()) ? "the deadline shown in your billing notice" : new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }).format(date);
}
...
function CustomerWorkspace({ children, pathname, isPolicyPublic }) {
  ...
  return <PullToRefresh>{!isPolicyPublic && <LegalAcceptanceGate />}<PaymentNotice /><NativeAppSetup />{!status.restricted && <HelpCenter />}{children}</PullToRefresh>;
}
...
export default function AppShell({ children }) {
  ...
  const utilityButtons = <div className="fixed bottom-[calc(0.75rem+env(safe-area-inset-bottom))] right-3 z-40 flex gap-2">{!isEmployee && pathname !== "/settings" && <Link href="/settings" className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-lg">Settings</Link>}{signOutButton}</div>;

  if (isEmployee) return <PullToRefresh><NativeAppSetup />{children}{utilityButtons}</PullToRefresh>;
  return <BillingStatusProvider><CustomerWorkspace pathname={pathname} isPolicyPublic={isPolicyPublic}>{children}{utilityButtons}</CustomerWorkspace></BillingStatusProvider>;
}
