...
function WorkspaceHeader({ profile, pathname, isEmployee, logout }) {
  ...
  return <header className="sticky top-0 z-[60] border-b border-slate-200 bg-white/95 px-3 py-3 shadow-sm backdrop-blur sm:px-5 md:px-8 md:py-4"><div className="mx-auto flex max-w-6xl items-center justify-between gap-3"><div className="min-w-0 leading-tight"><p className="truncate text-lg font-black tracking-tight text-slate-950 sm:text-2xl">ARK Client Center</p><p className="mt-0.5 truncate text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500 sm:text-xs">{subtitle}</p></div><div className="flex shrink-0 items-center gap-2"><button type="button" onClick={() => requestUnsavedNavigation("Sign Out", logout)} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm sm:px-4 sm:py-2.5">Sign out</button><Link href={settingsHref} aria-label="Settings" title="Settings" className={settingsActive ? "grid h-9 w-9 place-items-center rounded-xl bg-slate-950 text-lg text-white shadow-sm sm:h-10 sm:w-10" : "grid h-9 w-9 place-items-center rounded-xl border border-slate-300 bg-white text-lg text-slate-700 shadow-sm sm:h-10 sm:w-10"}><span aria-hidden="true">⚙</span></Link></div></div></header>;
}
