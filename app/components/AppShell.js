...
function LoadingScreen({ message = "Loading client center…" }) {
  return (
    <main className="grid min-h-screen place-items-center bg-slate-50 p-6">
      <div className="rounded-2xl border border-slate-200 bg-white px-8 py-6 text-sm font-semibold text-slate-600 shadow-sm">{message}</div>
    </main>
  );
}
...
function PullToRefresh({ children }) {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} onTouchCancel={handleTouchEnd}>
      ...
      <div style={{ transform: distance > 0 ? `translateY(${distance}px)` : "none", transition: tracking.current ? "none" : "transform 160ms ease-out" }}>{children}</div>
    </div>
  );
}
  ...
  return (
    <section className={restricted ? "border-b border-red-300 bg-red-50 px-3 py-4" : "border-b border-amber-300 bg-amber-50 px-3 py-4"}>
      <div className="mx-auto flex max-w-6xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><p className={restricted ? "text-sm font-black text-red-900" : "text-sm font-black text-amber-950"}>{title}</p><p className={restricted ? "mt-1 text-xs font-semibold leading-5 text-red-800" : "mt-1 text-xs font-semibold leading-5 text-amber-900"}>{body}</p>{status.offenseNumber > 0 && <p className="mt-1 text-[10px] font-black uppercase tracking-wide text-slate-600">Payment incident {status.offenseNumber} within the rolling six-month window</p>}{error && <p className="mt-1 text-xs font-bold text-red-700">{error}</p>}</div>
        <div className="grid shrink-0 grid-cols-2 gap-2"><button type="button" disabled={openingBilling} onClick={openBillingPortal} className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{openingBilling ? "Opening…" : "Update Payment"}</button><Link href="/terms#payment-enforcement" className="rounded-xl border border-slate-400 bg-white px-4 py-2.5 text-center text-xs font-black text-slate-800">Learn More</Link></div>
      </div>
    </section>
  );
}
...
function WorkspaceNav({ profile, pathname, employee = false }) {
  const plan = profile?.billingPlan || "solo";
  ...
  return (
    <nav className="border-b border-slate-200 bg-slate-50 px-3 py-2 sm:px-5 md:px-8">
      <div className={`mx-auto grid max-w-6xl gap-1 rounded-xl bg-slate-200/70 p-1 ${items.length === 3 ? "grid-cols-3" : "grid-cols-2"}`}>
        {items.map((item) => {
          const active = item.href === "/lead-messages" ? pathname.startsWith("/lead-messages") : item.href === "/employees" ? pathname.startsWith("/employees") : pathname === "/";
          return <Link key={item.label} href={item.href} className={active ? "rounded-lg bg-white px-3 py-2.5 text-center text-xs font-black text-slate-950 shadow-sm sm:text-sm" : "rounded-lg px-3 py-2.5 text-center text-xs font-bold text-slate-600 sm:text-sm"}>{item.label}</Link>;
        })}
      </div>
    </nav>
  );
    ...
    <PullToRefresh>
      <header className="border-b border-slate-200 bg-white px-3 py-3 shadow-sm sm:px-5 md:px-8 md:py-4">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          <div className="min-w-0 leading-tight"><p className="truncate text-lg font-black tracking-tight text-slate-950 sm:text-2xl">ARK Client Center</p><p className="mt-0.5 truncate text-xs font-bold uppercase tracking-[0.12em] text-slate-500 sm:text-sm">{accountLabel}</p></div>
          <div className="flex shrink-0 items-center gap-2">{signOutButton}{!status.restricted && <Link href="/settings" aria-label="Settings" title="Settings" className={settingsActive ? "grid h-10 w-10 place-items-center rounded-lg bg-slate-950 text-white shadow-sm sm:h-11 sm:w-11" : "grid h-10 w-10 place-items-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 hover:text-slate-950 sm:h-11 sm:w-11"}><span className="text-lg" aria-hidden="true">⚙</span></Link>}</div>
        </div>
      </header>
      {!status.restricted && <WorkspaceNav profile={profile} pathname={pathname} />}
...
function EmployeeShell({ children, pathname, signOutButton, profile }) {
  return (
    <PullToRefresh>
      <header className="border-b border-slate-200 bg-white px-3 py-3 shadow-sm sm:px-5 md:px-8 md:py-4">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          <div className="min-w-0 leading-tight"><p className="truncate text-lg font-black tracking-tight text-slate-950 sm:text-2xl">ARK Business Workspace</p><p className="mt-0.5 truncate text-xs font-bold uppercase tracking-[0.12em] text-slate-500 sm:text-sm">{profile?.businessName || "Business"} · {profile?.employeeName || "Employee"}</p></div>
          {signOutButton}
        </div>
      </header>
      <WorkspaceNav profile={profile} pathname={pathname} employee />
      {children}
    </PullToRefresh>
  );
}
...
export default function AppShell({ children }) {
      ...
      <>
        <header className="border-b border-slate-200 bg-white px-3 py-2.5 shadow-sm md:px-8 md:py-4">
          <div className="mx-auto flex max-w-7xl flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            <div className="flex min-w-0 items-center justify-between gap-3"><div className="min-w-0 leading-tight"><p className="truncate text-base font-black tracking-tight text-slate-950 sm:text-xl">ARK Client Center</p><p className="truncate text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500 sm:text-xs">Admin</p></div><div className="sm:hidden">{signOutButton}</div></div>
            <div className="flex min-w-0 items-center gap-2"><nav className="grid min-w-0 flex-1 gap-1 rounded-xl bg-slate-100 p-1 sm:flex sm:flex-none" style={{ gridTemplateColumns: `repeat(${ADMIN_NAV_ITEMS.length}, minmax(0, 1fr))` }}>{ADMIN_NAV_ITEMS.map((item) => { const active = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href); return <Link key={item.href} href={item.href} className={active ? "min-w-0 rounded-lg bg-white px-1 py-2 text-center text-[9px] font-black text-slate-950 shadow-sm sm:whitespace-nowrap sm:px-3 sm:text-sm" : "min-w-0 rounded-lg px-1 py-2 text-center text-[9px] font-bold text-slate-600 hover:bg-white/60 hover:text-slate-950 sm:whitespace-nowrap sm:px-3 sm:text-sm"}><span className="sm:hidden">{item.mobileLabel}</span><span className="hidden sm:inline">{item.label}</span></Link>; })}</nav><div className="hidden sm:block">{signOutButton}</div></div>
          </div>
        </header>
        {children}
      </>
  ...
  return (
    <BillingStatusProvider>
      <CustomerShell pathname={pathname} isPolicyPublic={isPolicyPublic} signOutButton={signOutButton} profile={profile}>{children}</CustomerShell>
    </BillingStatusProvider>
  );
}
