...
export default function ClientDeclineNoticeSettings({ embedded = false }) {
  const { user } = useAuth();
  ...
  return (
    <section className={embedded ? "mt-6 border-t border-slate-200 pt-6" : "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"}>
      ...
      <label htmlFor="customer-decision-texts" className={`mt-4 flex items-center justify-between gap-4 rounded-xl border border-slate-200 p-4 ${loading || saving ? "cursor-not-allowed bg-slate-50 opacity-60" : "cursor-pointer bg-white"}`}>
        ...
        <span aria-hidden="true" className={`relative h-7 w-12 shrink-0 rounded-full transition ${enabled ? "bg-[#071a3d]" : "bg-slate-300"}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow-sm transition ${enabled ? "left-6" : "left-1"}`} /></span>
      </label>
