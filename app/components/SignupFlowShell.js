...
function routeMatches(pathname, values) { return values.some((value) => pathname === value || pathname.startsWith(`${value}/`)); }
function Waiting({ children }) { return <main className="grid min-h-screen place-items-center bg-slate-50 p-6"><div className="rounded-2xl border border-slate-200 bg-white px-8 py-6 text-sm font-semibold text-slate-600 shadow-sm">{children}</div></main>; }
...
export default function SignupFlowShell({ children }) {
  ...
  useEffect(() => {
    ...
    if (user && isEmployee && profile?.status === "active" && employeePendingPage) { router.replace("/"); return; }
    if (user && !isAdmin && !isEmployee && profile?.status === "active" && pathname === "/signup/status") router.replace(needsBusinessSetup ? "/setup/business" : "/");
  ...
  if (ownerUnfinished) return <Waiting>Opening payment setup…</Waiting>;
  if (needsBusinessSetup && publicInformationPage) return children;
  if (needsBusinessSetup) return <Waiting>Opening account setup…</Waiting>;
