...
export default function SettingsPanel({ setupMode = false }) {
  ...
  const { user, profile, isAdmin, isBusinessOwner, refreshProfile } = useAuth();
  ...
  useEffect(() => {
    ...
    return onSnapshot(settingsRef, (snapshot) => setAccountSettings({ ...DEFAULT_SETTINGS, ...(snapshot.exists() ? snapshot.data() : {}) }), () => setError("Could not load this business's account information."));
  }, [clientId]);
    ...
    <main className="min-h-screen bg-slate-50 px-3 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] text-slate-950 sm:p-5 md:p-8">
      <div className="mx-auto max-w-4xl">
        <header className="mb-4 sm:mb-7">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{setupMode ? "Complete the business and AI receptionist settings below before entering the main app." : "Manage business information, billing, employees, downloads, help, and account resources."}</p>
        </header>
        ...
        <details defaultOpen={setupMode} className="group rounded-2xl border border-slate-200 bg-white shadow-sm sm:rounded-3xl">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-4 sm:p-6 md:p-8 [&::-webkit-details-marker]:hidden"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">AI receptionist</p><h2 className="mt-1 text-xl font-black sm:text-2xl">Business Information</h2><p className="mt-1 text-xs font-semibold leading-5 text-slate-500">View and edit the information and voice settings used during calls.</p></div><span className="shrink-0 rounded-xl border border-slate-300 px-3 py-2 text-xs font-black group-open:bg-slate-950 group-open:text-white">View and edit</span></summary>
        ...
        {!setupMode && <>
          {isBusinessOwner && <Link href="/employees#visibility" className="mt-4 block rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:mt-6 sm:rounded-3xl sm:p-6"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Business team</p><h2 className="mt-1 text-lg font-black sm:text-2xl">Employees and Access</h2><p className="mt-2 text-xs leading-5 text-slate-500 sm:text-sm">Approve employee accounts, decide which lead fields employees can see, and assign work.</p><span className="mt-4 inline-flex rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white">Configure Employees</span></Link>}
