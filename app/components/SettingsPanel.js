    ...
    <main className="min-h-screen bg-slate-50 px-3 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] text-slate-950 sm:p-5 md:p-8">
      <div className="mx-auto max-w-4xl">
        <header className="mb-4 sm:mb-7">
          {!setupMode && <Link href="/" className="mb-4 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm"><span aria-hidden="true">←</span>Back to Clients</Link>}
          ...
          <h1 className="mt-1 text-3xl font-black tracking-tight sm:text-4xl">{setupMode ? "Finish Business Setup" : "Business Information"}</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{setupMode ? "Complete the business and AI receptionist settings below before entering the main app." : "Update the business information and AI customization used during calls."}</p>
        </header>
