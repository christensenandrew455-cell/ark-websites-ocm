...
const SETTINGS_TILES = [
  { key: "business", title: "Business Information", description: "Business details, hours, services, and estimate availability.", symbol: "B" },
  { key: "customization", title: "Customization", description: "App tools, appearance, AI receptionist, and client-data download.", symbol: "C" },
  { key: "payment", title: "Payment", description: "Estimated monthly cost and payment method.", symbol: "$" },
  { key: "account", title: "Help & Account", description: "Help, documents, policies, support, and account deletion.", symbol: "?" },
];
...
function SettingsTile({ title, description, symbol, onClick }) {
  return (
    <button type="button" onClick={onClick} className="min-h-36 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition active:scale-[0.98] sm:min-h-44 sm:rounded-3xl sm:p-6">
      <span className="grid h-10 w-10 place-items-center rounded-xl bg-slate-950 text-sm font-black text-white sm:h-12 sm:w-12 sm:text-base">{symbol}</span>
      <h2 className="mt-4 text-base font-black text-slate-950 sm:text-xl">{title}</h2>
      <p className="mt-1 text-[11px] font-semibold leading-5 text-slate-500 sm:text-sm">{description}</p>
    </button>
  );
}
...
function SectionHeader({ title, description, onBack }) {
  return (
    <div className="mb-4 sm:mb-6">
      <button type="button" onClick={onBack} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm">
...
function SectionPanel({ children, danger = false }) {
  return <section className={danger ? "rounded-2xl border border-red-200 bg-red-50 p-4 shadow-sm sm:rounded-3xl sm:p-7" : "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-7"}>{children}</section>;
}
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-3 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] text-slate-950 sm:p-5 md:p-8">
      ...
      <div className="mx-auto max-w-4xl">
        <header className="mb-4 sm:mb-7">{!setupMode && <Link href="/" className="mb-4 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm"><span aria-hidden="true">←</span>Back to Dashboard</Link>}<p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{setupMode ? "Final account step" : "ARK Client Center"}</p><h1 className="mt-1 text-3xl font-black tracking-tight sm:text-4xl">{setupMode ? "Finish Account Setup" : "Settings"}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{setupMode ? "Complete the business and receptionist setup below." : activeSection ? "Change this section, then return to the Settings blocks." : "Choose one block."}</p></header>
        ...
        {setupMode ? (
          <SectionPanel>{isLoading || !receptionist ? <p className="rounded-xl border border-slate-200 p-5 text-center text-sm text-slate-500">Loading setup…</p> : <form onSubmit={saveBusinessInformation}><ReceptionistBusinessForm profile={receptionist} onChange={(next) => setReceptionist(next)} /><button type="submit" disabled={isSaving} className="mt-7 w-full rounded-xl bg-slate-950 px-6 py-3 text-sm font-black text-white disabled:opacity-50 sm:w-auto">{isSaving ? "Saving…" : "Save and Open Client Center"}</button></form>}</SectionPanel>
        ) : isOwner && !activeSection ? (
          <div className="grid grid-cols-2 gap-3 sm:gap-4">{SETTINGS_TILES.map((tile) => <SettingsTile key={tile.key} {...tile} onClick={() => setActiveSection(tile.key)} />)}</div>
        ) : isOwner && activeSection === "business" ? renderBusinessSection()
          : isOwner && activeSection === "customization" ? renderCustomizationSection()
            : isOwner && activeSection === "payment" ? renderPaymentSection()
              : isOwner && activeSection === "account" ? renderAccountSection()
                : null}
      </div>
    </main>
  );
