      ...
      <div className="mx-auto w-full max-w-4xl">
        <header className="mb-4 sm:mb-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <button type="button" onClick={goBack} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm hover:bg-slate-100 hover:text-slate-950 sm:text-sm"><span aria-hidden="true">←</span>Back</button>
        ...
        <nav className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          <div className="mt-3 grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2" style={{ width: "100%", maxWidth: "100%", marginLeft: 0, marginRight: 0 }}>
            {HELP_SECTIONS.map((section) => (
              <a key={section.id} href={`#${section.id}`} className="block w-full min-w-0 max-w-full overflow-hidden whitespace-normal break-words rounded-xl bg-slate-100 px-3 py-2.5 text-center text-[11px] font-black leading-4 text-slate-700 hover:bg-slate-200 sm:text-xs">{section.title}</a>
            ))}
          </div>
        </nav>
          ...
          {HELP_SECTIONS.map((section) => (
            <section key={section.id} id={section.id} className="min-w-0 scroll-mt-28 overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:rounded-3xl sm:p-8">
              ...
              <ul className="mt-4 space-y-2.5 text-sm leading-6 text-slate-600 sm:text-base sm:leading-7">
                {section.points.map((point) => <li key={point} className="flex min-w-0 gap-3"><span aria-hidden="true" className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-slate-950" /><span className="min-w-0 break-words">{point}</span></li>)}
              </ul>
              <div className="mt-5 flex min-w-0 flex-wrap gap-2">
                {section.links.map((label) => <Link key={label} href={linkMap.get(label) || "/docs"} className="max-w-full whitespace-normal break-words rounded-xl border border-slate-300 px-3 py-2 text-xs font-black text-slate-800 hover:bg-slate-50 sm:text-sm">{label}</Link>)}
              </div>
            </section>
          ))}
        </div>
