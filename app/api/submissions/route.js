...
export async function GET() {
  return NextResponse.json({
    ok: true,
    message: "ARK OCM submissions route is live.",
  });
}
...
export async function POST(request) {
  try {
    ...
    const clientId = cleanClientId(
      body.clientId || body.businessId || body.websiteId
    );

    const sectionKey =
      allowedSections[body.section] ||
      allowedSections[body.status] ||
      "preClients";
    ...
    if (!hasContactInfo) {
      return NextResponse.json(
        {
          error:
            "Missing submission info. Send at least name, phone, email, or notes.",
        },
        { status: 400 }
      );
    }
