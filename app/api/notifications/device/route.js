...
export async function POST(request) {
  ...
  try {
    ...
    const db = getAdminDb();
    const devices = db.collection("ocmClients").doc(auth.clientId).collection("notificationDevices");

    if (action === "mark-viewed") {
      const snapshot = await devices.where("uid", "==", auth.decodedToken.uid).get();
      const batch = db.batch();
      snapshot.docs.forEach((document) => {
        batch.set(document.ref, {
          unreadLeadCount: 0,
          lastViewedLeadsAt: FieldValue.serverTimestamp(),
    ...
    await reference.set({
      ...
      appVersion: text(body.appVersion || "1.1"),
