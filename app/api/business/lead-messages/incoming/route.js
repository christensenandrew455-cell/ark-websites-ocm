...
export async function POST(request) {
  if (!secretMatches(request)) {
    return NextResponse.json({ error: "Invalid messaging webhook credentials." }, { status: 401 });
  }
  ...
  try {
    ...
    if (!clientId || !conversationId || !messageBody) {
    ...
    if (!conversationSnapshot.exists) {
      return NextResponse.json({ error: "That lead conversation does not exist." }, { status: 404 });
    }
    ...
    batch.set(messageRef, {
      direction: "inbound",
      body: messageBody,
      senderName: text(body.senderName || conversationSnapshot.data().leadName || body.from),
      senderRole: "lead",
      providerMessageId: text(body.providerMessageId || body.messageId) || null,
      providerFrom: text(body.from) || null,
