...
function normalizeLead(document, collectionKey) {
  ...
  return { id: document.id, collectionKey, name: text(data.Name || data.name || data.fullName) || "Unnamed lead", phone: text(data.Phone || data.phone || data.phoneNumber), email: text(data.Email || data.email), job: text(data.Job || data.job || data.service || data.projectType), address: text(data.Address || data.address), assignedEmployeeUid: text(data.assignedEmployeeUid), assignedEmployeeName: text(data.assignedEmployeeName) };
}
...
async function authorizeMessaging(request) {
  ...
  const db = getAdminDb();
  const [accountSnapshot, businessSnapshot] = await Promise.all([db.collection("accounts").doc(decoded.uid).get(), db.collection("businesses").doc(clientId).get()]);
  ...
  return { db, decoded, account, business, clientId, isEmployee };
}
...
async function loadConversations(access) {
  ...
  return snapshot.docs.map((document) => {
    const data = document.data();
    return { id: document.id, leadId: text(data.leadId), collectionKey: text(data.collectionKey || "contactedMe"), leadName: text(data.leadName) || "Unnamed lead", leadPhone: text(data.leadPhone), assignedEmployeeUid: text(data.assignedEmployeeUid), assignedEmployeeName: text(data.assignedEmployeeName), lastMessage: text(data.lastMessage), lastMessageDirection: text(data.lastMessageDirection), unreadCount: access.isEmployee ? Number(data.employeeUnreadCount || 0) : Number(data.ownerUnreadCount || 0), lastMessageAt: iso(data.lastMessageAt || data.updatedAt || data.createdAt), createdAt: iso(data.createdAt) };
...
async function loadMessages(access, conversationKey) {
  ...
  return snapshot.docs.map((document) => { const data = document.data(); return { id: document.id, direction: text(data.direction || "outbound"), body: text(data.body), senderName: text(data.senderName), senderRole: text(data.senderRole), deliveryStatus: text(data.deliveryStatus), createdAt: iso(data.createdAt) }; });
}
...
export async function GET(request) {
  ...
  try {
    ...
    return NextResponse.json({ role: access.isEmployee ? "employee" : "owner", messagingConnected: Boolean(text(process.env.ARK_MESSAGING_WEBHOOK_URL)), availableLeads, conversations, selectedConversation, messages, unreadCount: conversations.reduce((total, item) => total + Number(item.unreadCount || 0), 0) });
  } catch (error) {
...
async function sendThroughProvider(payload) {
  ...
  try {
    const response = await fetch(webhookUrl, { method: "POST", headers: { "Content-Type": "application/json", ...(process.env.ARK_MESSAGING_WEBHOOK_SECRET ? { Authorization: `Bearer ${process.env.ARK_MESSAGING_WEBHOOK_SECRET}` } : {}) }, body: JSON.stringify(payload), cache: "no-store" });
  ...
  } catch (error) { return { status: "provider-error", providerMessageId: "", providerError: text(error.message) }; }
}
...
export async function POST(request) {
  ...
  try {
    ...
    const provider = await sendThroughProvider({ clientId: access.clientId, conversationId: key, leadId, collectionKey, to: loaded.lead.phone, leadName: loaded.lead.name, message: messageBody, senderUid: access.decoded.uid, senderRole: access.isEmployee ? "employee" : "owner" });
    ...
    batch.set(messageRef, { direction: "outbound", body: messageBody, senderUid: access.decoded.uid, senderName: text(access.account.employeeName || access.account.ownerName || access.account.accountEmail), senderRole: access.isEmployee ? "employee" : "owner", deliveryStatus: provider.status, providerMessageId: provider.providerMessageId || null, providerError: provider.providerError || null, createdAt: FieldValue.serverTimestamp() });
    batch.set(conversationRef, { conversationId: key, leadId, collectionKey, leadName: loaded.lead.name, leadPhone: loaded.lead.phone, assignedEmployeeUid: loaded.lead.assignedEmployeeUid || null, assignedEmployeeName: loaded.lead.assignedEmployeeName || null, lastMessage: messageBody, lastMessageDirection: "outbound", lastMessageAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp(), ...(!existingConversation.exists ? { startedByUid: access.decoded.uid, startedByRole: access.isEmployee ? "employee" : "owner", ownerUnreadCount: 0, employeeUnreadCount: 0, createdAt: FieldValue.serverTimestamp() } : {}) }, { merge: true });
    await batch.commit();
    return NextResponse.json({ ok: true, conversationId: key, newConversation: !existingConversation.exists, deliveryStatus: provider.status, notice: provider.status === "provider-not-configured" ? "The conversation was saved in ARK, but an outbound messaging provider is not connected yet." : provider.status === "sent" ? "Message sent." : "The conversation was saved, but the messaging provider reported an error." });
  } catch (error) {
