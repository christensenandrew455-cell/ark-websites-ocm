...
export async function GET(request) {
  ...
  const snapshot = await getAdminDb().collection("accounts").get();
  const applications = snapshot.docs
    .filter((document) => isStandardRole(document.data().role) && document.data().status === "active" && document.data().numberAssignmentStatus === "needed")
...
export async function POST(request) {
  ...
  try {
    ...
    const connectionKey = text(business.connectionKey) || randomBytes(24).toString("hex");
    const assigned = {
    ...
    const duplicateQuery = db.collection("accounts")
      .where("receptionistPhoneNormalized", "==", receptionistPhoneNormalized)
      .limit(2);
    await db.runTransaction(async (transaction) => {
      const [latestBusiness, duplicateSnapshot] = await Promise.all([
      ...
      if (duplicateSnapshot.docs.some((document) => document.id !== clientId)) throw new Error("PHONE_TAKEN");
