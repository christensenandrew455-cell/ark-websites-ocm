...
export default function SignupPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      sessionStorage.setItem(PENDING_SIGNUP_KEY, JSON.stringify(form));
    ...
    <main className="min-h-screen bg-slate-950 p-5 py-10">
      <div className="mx-auto w-full max-w-xl rounded-3xl bg-white p-7 shadow-2xl md:p-9">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Enter the business information, then add a card through Stripe. ARK OCM never sees or stores the card number.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-7 grid gap-4 md:grid-cols-2">
          <label className="block md:col-span-2">
            ...
            <input required name="businessName" value={form.businessName} onChange={updateField} className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-slate-950" />
          </label>
          <label className="block md:col-span-2">
          ...
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Account phone</span>
          ...
          <button disabled={submitting} className="rounded-xl bg-slate-950 px-5 py-3 font-bold text-white disabled:opacity-60 md:col-span-2">
            {submitting ? "Opening Stripe…" : "Continue to secure card setup"}
          </button>
        </form>

        <p className="mt-4 text-center text-xs text-slate-500">Your account is created only after Stripe confirms the card setup.</p>
