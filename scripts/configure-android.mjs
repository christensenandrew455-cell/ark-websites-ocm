...
const mainActivityMatch = manifest.match(mainActivityPattern);
if (!mainActivityMatch) {
  throw new Error("AndroidManifest.xml does not contain the Capacitor MainActivity.");
}
if (!/android:screenOrientation=/.test(mainActivityMatch[1])) {
  manifest = manifest.replace(mainActivityPattern, `<activity$1 android:screenOrientation="portrait">`);
  manifestChanged = true;
  console.log("[Android configuration] Locked MainActivity to portrait orientation.");
} else if (!/android:screenOrientation=["']portrait["']/.test(mainActivityMatch[1])) {
  manifest = manifest.replace(mainActivityPattern, (activity) => activity.replace(/android:screenOrientation=["'][^"']+["']/, 'android:screenOrientation="portrait"'));
  manifestChanged = true;
  console.log("[Android configuration] Updated MainActivity to portrait orientation.");
} else {
  console.log("[Android configuration] Portrait orientation already configured.");
}

if (manifestChanged) await writeFile(manifestPath, manifest, "utf8");
...
<vector xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <group
        android:translateX="21.60"
        android:translateY="32.10"
        android:scaleX="0.60"
        android:scaleY="0.59">
        <path
